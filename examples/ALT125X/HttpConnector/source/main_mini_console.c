...
#define MAX_CERT_BUFSIZE 4 * 1024

uint8_t *credBuf = NULL;
uint16_t credDataLen = 0;

extern int apitest_main(int argc, char *argv[]);

bool param_field_alloc(char *argv[]) {
  int i;
...
void param_field_free(char *argv[]) {
  int i;
...
int parse_arg(char *s, char *argv[]) {
...
int do_certpaste(char *s) {
  ...
  if (NULL == (credBuf = malloc(MAX_CERT_BUFSIZE))) {
    printf("malloc failed\r\n");
    return 0;
  }

  printf(
      "Paste mode\r\nPlease paste the certficate(Note: UNIX LF Format)\r\nPress CTRL+D to end the "
      "paste mode\r\n");
  idx = 0;
  ...
  while (true) {
    ...
    if (MAX_CERT_BUFSIZE - 1 <= idx) {
      credBuf[MAX_CERT_BUFSIZE - 1] = '\0';
      credDataLen = MAX_CERT_BUFSIZE - 1;
      break;
    } else {
      credBuf[idx] = c;
    }
  ...
  printf("\r\nPaste mode end, credential length %d.\r\n", idx);

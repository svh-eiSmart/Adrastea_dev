...
#define LTEAPI_CMD_NUM_OF_PARAM_MAX 12
#define LTEAPI_CMD_PARAM_LENGTH 128

static bool param_field_alloc(char *argv[]) {
  int i;

  for (i = 0; i < LTEAPI_CMD_NUM_OF_PARAM_MAX; i++) {
    argv[i] = malloc(LTEAPI_CMD_PARAM_LENGTH);
    if (!argv[i]) {
      printf("cmd field alloc failed.\n");
      return false;
    }
    memset(argv[i], 0, LTEAPI_CMD_PARAM_LENGTH);
  }
  return true;
}
...
static int32_t parse_arg(char *s, char *argv[]) {
  int argc = 1; /* At least, include "lteapi" or other */
  char *tmp_s;
  int i;

  /* Parse arg */
  if (strlen(s)) {
    for (i = 1; i < LTEAPI_CMD_NUM_OF_PARAM_MAX; i++) {
      tmp_s = strchr(s, ' ');
      if (!tmp_s) {
        tmp_s = strchr(s, '\0');
        if (!tmp_s) {
...
int do_apitest(char *s) {
  int32_t argc;
  char *argv[LTEAPI_CMD_NUM_OF_PARAM_MAX] = {0};

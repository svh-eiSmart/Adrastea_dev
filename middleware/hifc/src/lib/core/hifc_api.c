...
void hifc_protocol_timeout_isr(alt_osal_timer_handle timer);
void hifc_inactivity_timeout_isr(alt_osal_timer_handle timer);
...
struct hifc_priv {
  ...
  alt_osal_timer_handle xInactivity_Timer;
  alt_osal_timer_handle protcol_timeout_timer;
  alt_osal_timer_handle cmode_resume_timer;
...
void hifc_cmode_resume_time_isr(alt_osal_timer_handle timer) {
  set_if_state(IfUp);
...
void hifc_inactivity_timeout_isr(alt_osal_timer_handle timer) {
  if (isUartTraffic) {
    ...
    alt_osal_start_timer(&gHifc_struct.xInactivity_Timer, gInact_tout_anchor.period);
  } else {
...
static enum hifc_fsm hifc_host_resume_sm(void) {
  ...
  switch (hifc_get_host_interface_state()) {
    case (hifc_fsm_init):
      ...
      if (mode == hifc_mode_a) {
        ...
        hifc_set_host_interface_state(hifc_fsm_config);
        gHifc_mi_cb.h2m_set_cb(set);
        alt_osal_start_timer(&gHifc_struct.protcol_timeout_timer, PROTOCOL_TIMEOUT);
    ...
    case (hifc_fsm_config):
      ...
      alt_osal_stop_timer(&gHifc_struct.protcol_timeout_timer);
      hifc_set_host_interface_state(hifc_fsm_approve);
      gHifc_mi_cb.m2h_init_cb(hifc_m2h_level_change_callback, set);
  ...
  if (hifc_get_host_interface_state() == hifc_fsm_complete) {
    ...
    if (mode == hifc_mode_a || mode == hifc_mode_b) {
      ...
      alt_osal_start_timer(&gHifc_struct.cmode_resume_timer, CMODE_RESUME_TIME);
    }
  }
...
static int32_t hifc_host_resume(hifc_sys_lpuart_t* base) {
  ...
  if (!isHifcPort(base)) return -EWONTDO;
  if (get_if_state() != IfDown) return -EALRDYDONE;
...
int32_t hifc_host_interface_init(hifc_sys_lpuart_t* uart_base) {
  ...
  if (!gHifc_struct.uart_base) {
    ...
    if (alt_osal_create_timer(&gHifc_struct.protcol_timeout_timer, false, hifc_protocol_timeout_isr,
                              NULL, NULL)) {
      return -ENOMEM;
    }
    if (alt_osal_create_timer(&gHifc_struct.xInactivity_Timer, false, hifc_inactivity_timeout_isr,
                              NULL, NULL)) {
      return -ENOMEM;
    }
    if (alt_osal_create_timer(&gHifc_struct.cmode_resume_timer, false, hifc_cmode_resume_time_isr,
                              NULL, NULL)) {
      return -ENOMEM;
    }
  }
...
void set_hifc_mode(enum hifc_mode mode) {
  ...
  if (gHifc_struct.eMode != hifc_mode_off) {
    return;
  }
...
uint32_t isHifcPort(hifc_sys_lpuart_t* base) {
  ...
  if (get_hifc_mode() == hifc_mode_off) {
    return 0;
  }
...
int32_t stop_inactivity_timer(hifc_sys_lpuart_t* base) {
  ...
  if (get_if_state() == IfUp && gHifc_struct.uart_base == base) {
    alt_osal_stop_timer(&gHifc_struct.xInactivity_Timer);
  }
  return 0;
}
...
int32_t set_inactivity_timer(uint32_t ms) {
  ...
  if (get_if_state() == IfDown) {
    get_reset_timer_period(gHifc_struct.inactivity_time);
    if (alt_osal_start_timer(&gHifc_struct.xInactivity_Timer, gInact_tout_anchor.period)) {
      return -EBUSY;
    }
    if (alt_osal_stop_timer(&gHifc_struct.xInactivity_Timer)) {
      return -EBUSY;
    }
    ...
    if (generic_reset_inactivity_timer(gHifc_struct.uart_base, true)) return EBUSY;
...
int32_t force_host_suspend(void) {
  ...
  if (get_if_state() == IfUp) {
    ret = alt_osal_stop_timer(&gHifc_struct.xInactivity_Timer);
    if (ret) {
      goto error;
    }
...
enum hifc_request_to_send request_to_send(hifc_sys_lpuart_t* base) {
  ...
  if (is_hifc_port) {
    ...
    alt_osal_enter_critital_section();
    ...
    alt_osal_exit_critital_section();
  }
  return ret;
}

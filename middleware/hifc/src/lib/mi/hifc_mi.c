...
void MAILBOX_IRQHandler(uint32_t user_param) {
  uint32_t val;

  val = DRV_ATOM_CNTR_GetCount(HIFC_INTERNAL_MODEM_2_HOST);
...
void hifc_mi_h2m_set(enum gpio_state e_state) {
  uint32_t val = DRV_ATOM_CNTR_GetCount(HIFC_INTERNAL_HOST_2_MODEM);

  if (e_state == set) {
    if (val == 0) {
      DRV_ATOM_CNTR_Increment(HIFC_INTERNAL_HOST_2_MODEM, 1);
    }
  } else {
    if (val == 1) {
      DRV_ATOM_CNTR_Decrement(HIFC_INTERNAL_HOST_2_MODEM, 1);
    }
  }
}
...
void hifc_mi_m2h_init(hifc_level_change_callback callback, enum gpio_state wait_for) {
  ...
  if (wait_for == clear)
    DRV_ATOM_CNTR_SetIrqMode(HIFC_INTERNAL_MODEM_2_HOST, ATOMIC_COUNTER_DEC_INTERRUPT_MODE_VALUE);
  // PORT_SetPinInterruptConfig(PORTD, M2H_PIN, kPORT_InterruptLogicZero
  // /*kPORT_InterruptFallingEdge*/);
  else
    DRV_ATOM_CNTR_SetIrqMode(HIFC_INTERNAL_MODEM_2_HOST,
  ...
  DRV_ATOM_CNTR_EnableIRQ(HIFC_INTERNAL_MODEM_2_HOST);
}
...
/* Unused function */
void hifc_mi_utx_set(enum gpio_state e_state) { /* internal MCU doen't need this function*/
}
void hifc_mi_urx_init(hifc_level_change_callback callback,
                      enum gpio_state wait_for) { /* internal MCU doen't need this function*/
}
void hifc_mi_urx_isr_deinit(void) { /* internal MCU doen't need this function*/
}
void hifc_mi_txrx_to_uart(void) { /* internal MCU doen't need this function*/
}
void hifc_mi_txrx_to_gpio(void) { /* internal MCU doen't need this function*/
}
void hifc_mi_disableRts(void) { /* internal MCU doen't need this function*/
}
void hifc_mi_enableRts(void) { /* internal MCU doen't need this function*/
}
void enable_TxDataRegEmptyInterrupt(
    hifc_sys_lpuart_t *base) { /* internal MCU doen't need this function*/
}
void hifc_mi_enableTxRx(hifc_sys_lpuart_t *base) { /* internal MCU doen't need this function*/
}
void hifc_mi_disableTxRx(hifc_sys_lpuart_t *base) { /* internal MCU doen't need this function*/
}
void rst_ind_isr_on(
    reset_indication_mi_callback callback) { /* internal MCU doen't need this function*/
}
void rst_ind_isr_off(void) { /* internal MCU doen't need this function*/
}

uint32_t is_hwflow_on(hifc_sys_lpuart_t *base) { return -1; }

uint32_t hifc_mi_is_tx_empty(hifc_sys_lpuart_t *base) { return DRV_UART_Is_Uart_Busy(base); }

uint32_t get_pin_value(uint32_t *port, uint32_t pin) {
  if (pin == HIFC_INTERNAL_MODEM_2_HOST) {
    return DRV_ATOM_CNTR_GetCount(HIFC_INTERNAL_MODEM_2_HOST);
  } else if (pin == HIFC_INTERNAL_HOST_2_MODEM) {
    return DRV_ATOM_CNTR_GetCount(HIFC_INTERNAL_HOST_2_MODEM);
  }
  return -1;
}
...
void init_mi(struct hifc_mi_callbacks *callbacks) {
  ...
  DRV_ATOM_CNTR_InitCounter(HIFC_INTERNAL_HOST_2_MODEM, 0, 0);
  DRV_ATOM_CNTR_InitCounter(HIFC_INTERNAL_MODEM_2_HOST, MAILBOX_IRQHandler, 0);
}

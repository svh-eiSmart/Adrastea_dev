...
/**
 * @defgroup ltesms SMS Library Interface Connector APIs
...
/** Message read Max user data length. */

#define ALTCOM_SMS_READ_USERDATA_MAXLEN (320)
...
struct altcom_sms_status_report_s {
  ...
  /**
   * If the status category is @ref ALTCOM_SMS_DELIVERY_STAT_CAT_OK ,
...
struct altcom_sms_msg_s {
  ...
  /** union of all 3 type of SMS message - based on the type */

  union {
    /** Send message. See @ref altcom_sms_submit_s */
...
typedef int (*sms_recv_cb_t)(struct altcom_sms_msg_s *msg, uint16_t index, void *userPriv);
...
typedef enum {
  SMS_RESULT_OK = 0,     /**< Result code on success */
  SMS_RESULT_ERROR = 1,  /**< Result code on failure */
  SMS_RESULT_CANCEL = 2, /**< Result code on cancel */
} smsresult_e;
 ...
 *
 * @return On success, SMS_RESULT_OK is returned. On failure,
 * SMS_RESULT_ERR is returned.
 */
 ...
 *
 * @param [out] mr_list_len: the resulting mr_list length
 *
 *
 * @return On success, SMS_RESULT_OK is returned. On failure,
 *          SMS_RESULT_ERR is returned.
...
smsresult_e altcom_sms_send(struct altcom_sms_msg_s *msg,
                            uint8_t mr_list[ALTCOM_SMS_MSG_REF_ID_MAX_NUM], uint32_t *mr_list_len);

/**
 * Read SMS message.
 *
 * @param [in] index: Index number of the storage to read.
 *
 * @param [out] msg: The pointer of structure for read message.
 *
 * @return On success, SMS_RESULT_OK is returned. On failure,
 *          SMS_RESULT_ERR is returned.
 */

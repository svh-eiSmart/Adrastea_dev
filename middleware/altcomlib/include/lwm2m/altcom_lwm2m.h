...
/**
 * @brief
 * When the %LWM2MOBJCMDU: "READ" URC arrival, host will generate resource data to response to
 * the read request, and this callback is used to free these generated resource data.
 *
 * @param[in] uridata: resource data.
 * @param[in] instCnt: Instance count of resource.
 */
typedef void (*lwm2m_uridata_free_cb_t)(void *uridata, uint32_t instCnt);

/**
 * @brief
 * When the %LWM2MOBJCMDU: "READ" URC arrival, it is
 ...
 * @param[in] numOfUriData: Amount of data in uridata.
 * @param[out] uridata: uridata array for read.
 * @param[out] freecb: Callback to free uridata.
 * @param[in] userPriv: User's private data.
 *
 * @return >0: amount of resource, <=0: failed to read data.
 */

 ...
 * @param [in] precedence: The precednece of IP type to resolve the hostname. See @ref
 ...
 * @param [out] srcport: Optinal buffer. If the underlying protocol provides the source port number
 * of the message, that source port number is placed in srcport.
 *
 * @return On success, it returns the number of bytes received. Otherwise, @ref SocketError_e is
 * returned.
 */
...
/**
 * @brief altcom_atsocket_delete() delete a socket. Deleting an activated socket is allowed. Don't
 * delete a TLS-enabled socket if you want to resume the TLS session.
 *
 * @param [in] sockid: The socket identifier.
 *
 * @return SOCKET_OK on success, SOCKET_ERR otherwise.

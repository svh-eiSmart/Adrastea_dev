 ...
 ****************************************************************************//**
 * @defgroup coapcfg COAP Configuration Constants
 * @{
 */

#define COAP_URC_TOKEN_LENGTH (9)       /**< COAP token length */
#define COAP_URC_DATA_LENGTH (1024)     /**< COAP data length */
#define COAP_URC_OPT_VALUE_LENGTH (512) /**< COAP optional value length */
#define COAP_URC_OPT_ID_LENGTH (128)    /**< COAP optional ID length */
...
typedef struct {
  CoapCfgIp_t coapCfgIp;     /**< Destination(server) IP configuration */
  CoapCfgDtls_t coapCfgDtls; /**< DTLS configuration */
  ...
  int protocols; /**< Connection protocol */
...
typedef enum {
  ...
  COAP_PREDEF_OPTION_URI_PORT = 7,
  COAP_PREDEF_OPTION_LOCATION_PATH = 8,
  COAP_PREDEF_OPTION_URI_PATH = 11,
  COAP_PREDEF_OPTION_CONTENT_TYPE = 12,
  COAP_PREDEF_OPTION_MAX_AGE = 14,
  COAP_PREDEF_OPTION_URI_QUERY = 15,
  COAP_PREDEF_OPTION_ACCEPT = 17,
  COAP_PREDEF_OPTION_TOKEN = 19,
  COAP_PREDEF_OPTION_LOCATION_QUERY = 20,
  COAP_PREDEF_OPTION_BLOCK2 = 23,
  COAP_PREDEF_OPTION_BLOCK1 = 27,
  COAP_PREDEF_OPTION_SIZE = 28,
  COAP_PREDEF_OPTION_PROXY_QUERY = 35,
  COAP_PREDEF_OPTION_PROXY_SCHEME = 39,
  COAP_PREDEF_OPTION_SIZE1 = 60,
...
typedef struct {
  unsigned char *token;       /**< COAP token */
  unsigned char token_length; /**< COAP token length */
} CoapToken_t;
...
#define COAP_DATA_MAXLEN (2048 + 1) /**< Maximum COAP data length */
...
typedef struct {
  ...
  char opt_value[COAP_URC_OPT_VALUE_LENGTH]; /**< Option by value.
    Format: Option val0 + null + Option val1 + null ....*/

  char opt_id[COAP_URC_OPT_ID_LENGTH]; /**< Option by id.
...
/**
 * @brief Definition of command event callback function upon messages arrival.
 *
 * @param [in] profile: Profile of command event.
 * @param [in] cbdata: Callback data of event.
 */
typedef void (*coapCmdEvCb)(unsigned char profile, CoapCallBackData_t *cbdata, void *priv);
/**
 * @brief Definition of reset event callback function upon messages arrival.
 *
 * @param [in] profile: Profile of reset event.
 * @param [in] token: Token of reset event.
 */
typedef void (*coapRstEvCb)(unsigned char profile, char *token, void *priv);
/**
 * @brief Definition of terminate event callback function upon messages arrival.
 *
 * @param [in] profile: Profile of terminate event.
 * @param [in] err: Err reason of terminate event.
 * @param [in] token: Token of terminate event.
 */
typedef void (*coapTermEvCb)(unsigned char profile, Coap_ev_cmdterm_e err, char *token, void *priv);
...
/**
  ...
  @param [in] profileId:            Assigned profile between 1 and 5.
  @param [out] context:			Retrieved profile configuration.
...
/**
  ...
  @param [in] profileId:            Assigned profile between 1 and 5.
  @param [in] context:				Profile configuration to save.

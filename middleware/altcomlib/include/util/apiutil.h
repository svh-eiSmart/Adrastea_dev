...
#define altcom_lock()            \
  do {                           \
    alt_osal_disable_dispatch(); \
  } while (0)
#define altcom_unlock()         \
  do {                          \
    alt_osal_enable_dispatch(); \
  } while (0)
...
extern alt_osal_mutex_handle g_altcom_apicallback_mtx;
...
static inline void altcom_callback_createlock(void) {
  int32_t ret;
  alt_osal_mutex_attribute param = {0};

  ret = alt_osal_create_mutex(&g_altcom_apicallback_mtx, &param);
  if (ret < 0) {
...
static inline void altcom_callback_deletelock(void) {
  ...
  ret = alt_osal_delete_mutex(&g_altcom_apicallback_mtx);
  if (ret < 0) {
...
bool altcom_sock_alloc_cmdandresbuff(FAR void **buff, int32_t id, uint16_t bufflen, FAR void **res,
                                     uint16_t reslen);
void altcom_mbedtls_free_cmdandresbuff(FAR void *cmdbuff, FAR void *resbuff);
bool altcom_mbedtls_alloc_cmdandresbuff(FAR void **buff, int32_t id, uint16_t bufflen,

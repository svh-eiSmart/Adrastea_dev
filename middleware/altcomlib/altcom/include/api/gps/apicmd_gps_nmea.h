...
struct apicmd_gpsraw_repevt_s {
  char nmea_msg[NMEA_RAW_MAX_LEN];  // raw NMEA message
} end_packed_struct;

struct apicmd_gpsurc_repevt_s{
char msg[URC_RAW_MAX_LEN]; 	//  Raw URC string message
};
...
struct apicmd_event_s {
  ...
  union {
    ...
    struct apicmd_gpsraw_repevt_s raw;
    uint8_t sessionst; /**< Session status */
    uint8_t allowst;   /**< Allow status */

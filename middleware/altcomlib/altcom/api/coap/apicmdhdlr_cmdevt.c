...
static void cmd_urc_handling_job(void *arg) {
  ...
  urc_data->tokenLen = ntohs(incoming->tokenLen);
  ...
  /* Token */
  if (str_incoming && urc_data->tokenLen) {
    snprintf(urc_data->token, COAP_URC_TOKEN_LENGTH, str_incoming);
    str_incoming += urc_data->tokenLen + NULLCHAR;
  }
  ...
  /* Setup callback */
  ALTCOM_GET_CALLBACK(g_coapCmdEvCb, callback, g_coapCmdEvCbPriv, cbpriv);
  if (!callback) {
    DBGIF_LOG_DEBUG("g_coapCmdEvCb is not registered.\n");
    goto release;
  }

  callback(incoming->profileId, urc_data, cbpriv);

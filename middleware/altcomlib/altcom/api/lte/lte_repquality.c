...
#define GET_QUALITY_2G_RESP_LEN (sizeof(struct apicmd_cmddat_repquality_2g_s))
...
extern quality_2g_report_cb_t g_quality_2g_callback;
extern void *g_quality_2g_cbpriv;
...
int32_t lte_set_report_quality_2g(quality_2g_report_cb_t quality_callback, uint32_t period,
  ...
  if (altcom_isCbRegOnly()) {
    ALTCOM_CLR_CALLBACK(g_quality_callback, g_quality_cbpriv);
    if (quality_callback) {
      ALTCOM_REG_CALLBACK(ret, g_quality_2g_callback, quality_callback, g_quality_2g_cbpriv,
                          userPriv);
    }
  ...
  cmdbuff = (FAR struct apicmd_cmddat_setrepquality_s *)apicmdgw_cmd_allocbuff(
      APICMDID_SET_REP_QUALITY_2G, QUALITY_DATA_LEN);
  if (!cmdbuff) {
    DBGIF_LOG_ERROR("Failed to allocate command buffer.\n");
    g_lte_setrepquality_isproc = false;
    return -ENOSPC;
...
lteresult_e lte_get_quality_2g(lte_quality_2g_t *quality) {
  ...
  uint16_t resLen = 0;
  FAR void *cmd;
  FAR struct apicmd_cmddat_repquality_2g_s *res;
  ...
  /* Allocate API command buffer to send */
  if (altcom_generic_alloc_cmdandresbuff((FAR void **)&cmd, APICMDID_GET_QUALITY_2G,
                                         GET_QUALITY_DATA_LEN, (FAR void **)&res,
                                         GET_QUALITY_2G_RESP_LEN)) {
    /* Send API command to modem */
    ret = apicmdgw_send((FAR uint8_t *)cmd, (FAR uint8_t *)res, GET_QUALITY_2G_RESP_LEN, &resLen,
                        ALT_OSAL_TIMEO_FEVR);
  } else {
    DBGIF_LOG_ERROR("Failed to allocate command buffer.\n");
    return LTE_RESULT_ERROR;
  ...
  if (GET_QUALITY_2G_RESP_LEN != resLen) {
    DBGIF_LOG1_ERROR("Unexpected response data length: %hu\n", resLen);
    ret = LTE_RESULT_ERROR;
    goto errout_with_cmdfree;
  }

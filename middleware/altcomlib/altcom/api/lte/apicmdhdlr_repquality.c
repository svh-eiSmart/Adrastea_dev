...
static void repquality_2g_job(FAR void *arg) {
  FAR struct apicmd_cmddat_repquality_2g_s *data;
  lte_quality_2g_t *repdat = NULL;
  quality_2g_report_cb_t callback;
  void *cbpriv;

  ALTCOM_GET_CALLBACK(g_quality_2g_callback, callback, g_quality_2g_cbpriv, cbpriv);
  if (!callback) {
    DBGIF_LOG_ERROR("g_cellinfo_callback is not registered.\n");
    return;
  }

  data = (FAR struct apicmd_cmddat_repquality_2g_s *)arg;

  repdat = (lte_quality_2g_t *)BUFFPOOL_ALLOC(sizeof(lte_quality_2g_t));
  if (!repdat) {
    DBGIF_LOG_ERROR("report data buffer alloc error.\n");
  } else {
    repdat->valid = APICMD_REP_QUALITY_ENABLE == data->enability ? LTE_VALID : LTE_INVALID;
    if (repdat->valid) {
      repdat->rssi = data->rssi;
      repdat->ber = data->ber;
    }
...
enum evthdlrc_e apicmdhdlr_repquality_2g(FAR uint8_t *evt, uint32_t evlen) {
  return apicmdhdlrbs_do_runjob(evt, APICMDID_REPORT_QUALITY_2G, repquality_2g_job);
}

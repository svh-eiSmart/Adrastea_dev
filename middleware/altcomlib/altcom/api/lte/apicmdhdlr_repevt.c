...
extern pdn_actevt_report_cb_t g_pdn_actevt_report_callback;
extern void *g_pdn_actevt_report_cbpriv;
...
static void repevt_dynpsm_report(FAR struct apicmd_cmddat_repdynpsm_s *dynPsmSetting) {
  ...
  setting.func = LTE_PSM_ENABLE_FORCE_OFF;
...
static void repevt_connphase_report(FAR struct apicmd_cmddat_repconnphase_s *connphase) {
  ...
  switch (connphase->scanreason) {
    ...
    case APICMD_REP_CONPHASE_SCAN_REASON_REGULAR_SCAN: {
      scanreason = LTE_CONPHASE_SCAN_REASON_REGULAR_SCAN;
    } break;
    case APICMD_REP_CONPHASE_SCAN_REASON_RESELECT_SCAN: {
      scanreason = LTE_CONPHASE_SCAN_REASON_RESELECT_SCAN;
    } break;
...
/****************************************************************************
 * Name: repevt_pdn_actevt_report
 *
 * Description:
 *   This function is an API callback for reporting PDN activation event.
 *
 * Input Parameters:
 *  fw_upgrade_evt    Pointer to received fw upgrade event.
 *
...
static void repevt_pdn_actevt_report(FAR struct apicmd_cmddat_pdnactevt_s *pdn_actevt) {
  pdn_actevt_report_cb_t callback;
  void *cbpriv;

  ALTCOM_GET_CALLBACK(g_pdn_actevt_report_callback, callback, g_pdn_actevt_report_cbpriv, cbpriv);
  if (!callback) {
    DBGIF_LOG_DEBUG("g_pdn_actevt_report_callback is not registered.\n");
    return;
  }

  callback(pdn_actevt->cid, pdn_actevt->event, cbpriv);
}
...
static void repevt_job(FAR void *arg) {
  ...
  switch (ntohs(data->type)) {
    ...
    case APICMD_REPORT_EVT_TYPE_PDN_ACTEVT: {
      repevt_pdn_actevt_report(&data->u.pdnactevt);
    } break;
    default: {

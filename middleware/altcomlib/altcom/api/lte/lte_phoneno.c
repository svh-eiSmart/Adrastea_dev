...
#define GETPHONENO_RESP_LEN (sizeof(struct apicmd_cmddat_phonenores_s))
...
lteresult_e lte_get_phoneno(lteerrcause_e *errcause, int8_t *phoneno) {
  ...
  uint16_t resLen = 0;
  FAR void *cmd;
  FAR struct apicmd_cmddat_phonenores_s *res;
  ...
  if (altcom_generic_alloc_cmdandresbuff((FAR void **)&cmd, APICMDID_GET_PHONENO,
                                         GETPHONENO_DATA_LEN, (FAR void **)&res,
                                         GETPHONENO_RESP_LEN)) {
    /* Send API command to modem */
    ret = apicmdgw_send((FAR uint8_t *)cmd, (FAR uint8_t *)res, GETPHONENO_RESP_LEN, &resLen,
                        ALT_OSAL_TIMEO_FEVR);
  } else {
  ...
  if (APICMD_PHONENO_RES_OK == ret) {
    strncpy((char *)phoneno, (char *)res->phoneno, LTE_MAX_PHONENUM_LEN);
  } else {

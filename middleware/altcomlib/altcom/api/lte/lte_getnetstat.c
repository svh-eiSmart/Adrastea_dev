...
#define GETNETSTAT_RESP_LEN (sizeof(struct apicmd_cmddat_getnetstatres_s))
...
lteresult_e lte_get_netstat(ltenetstate_e *state) {
  ...
  uint16_t resLen = 0;
  FAR void *cmd;
  FAR struct apicmd_cmddat_getnetstatres_s *res;
  ...
  if (altcom_generic_alloc_cmdandresbuff((FAR void **)&cmd, APICMDID_GET_NETSTAT,
                                         GETNETSTAT_DATA_LEN, (FAR void **)&res,
                                         GETNETSTAT_RESP_LEN)) {
    /* Send API command to modem */
    ret = apicmdgw_send((FAR uint8_t *)cmd, (FAR uint8_t *)res, GETNETSTAT_RESP_LEN, &resLen,
                        ALT_OSAL_TIMEO_FEVR);
  } else {
  ...
  *state = APICMD_GETNETSTAT_RES_CNCT == res->state ? LTE_NETSTAT_ATTACH : LTE_NETSTAT_DETACH;

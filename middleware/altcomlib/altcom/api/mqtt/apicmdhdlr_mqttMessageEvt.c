...
void mqttCheckInitCbCtrl(void) {
  ...
  if (!isMsgCbTblInit) {
    ...
    ret = alt_osal_create_mutex(&gMqttMsgCbCtrl.tblMtx, NULL);
...
static void mqttMessageEvt_job(FAR void *arg) {
  ...
  alt_osal_lock_mutex(&gMqttMsgCbCtrl.tblMtx, ALT_OSAL_TIMEO_FEVR);
  ...
  alt_osal_unlock_mutex(&gMqttMsgCbCtrl.tblMtx);
  if (callback) {

...
static LWM2MError_e lwm2mSendNotify(client_inst_t client, char *token, bool askForAckFromServer,
  ...
  DBGIF_LOG1_DEBUG("Notify datalen: %lu\n", datalen);
  if (!altcom_generic_alloc_cmdandresbuff((FAR void **)&cmd, APICMDID_LWM2M_NOTIFY,
                                          LWM2MNOTIFY_REQ_DATALEN + datalen, (FAR void **)&res,
                                          LWM2MNOTIFY_RES_DATALEN)) {
  ...
  for (numOfUriDataIdx = 0; numOfUriDataIdx < numOfUriData; numOfUriDataIdx++) {
    // Printout debug msg
    DBGIF_LOG3_DEBUG("lwm2mSendNotify objectId=%ld instnum=%ld resource=%ld ",
    ...
    if (uriAndData[numOfUriDataIdx].valType == RSRC_VALUE_TYPE_OPAQUE) {
      ...
      DBGIF_LOG1_DEBUG("string sent in Notify=\"%s\"\r\n",

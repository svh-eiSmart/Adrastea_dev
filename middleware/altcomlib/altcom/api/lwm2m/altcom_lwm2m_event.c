...
static void repevt_observe_start_report(FAR lwm2m_event_int_t *event) {
  ...
  if (callback) {
    ...
    snprintf(lwm2m_event.u.observeInfo.token, LWM2M_OBSERVE_TOKEN_STR_LEN + 1, "%s",
             event->u.lwm2m_obserStartReq.token);
...
static void repevt_observe_stop_report(FAR lwm2m_event_int_t *event) {
  ...
  if (callback) {
    ...
    lwm2m_event.u.observeInfo.rsrcInstId = rsrcInstId;
    snprintf(lwm2m_event.u.observeInfo.token, LWM2M_OBSERVE_TOKEN_STR_LEN + 1, "%s",
             event->u.lwm2m_obserStopReq.token);
    callback(client, &lwm2m_event, cbpriv);
  }
}
...
static void repevt_readReq_report(FAR lwm2m_event_int_t *event) {
  ...
  lwm2m_uridata_free_cb_t freecb = NULL;
  lwm2m_uri_and_data_t *uridata = NULL;
  void *cbpriv;
  ...
  if (callback) {
    instCnt = callback(client, objectId, instanceNum, resourceNum, &uridata, &freecb, cbpriv);
    if (instCnt <= 0) {
      readResult = false;
    }
  } else {
  ...
  altcom_lwm2mReadResp(client, ntohl(event->u.lwm2m_readReq.seqNum), readResult, (uint8_t)instCnt,
                       uridata);
  if (freecb && instCnt > 0) {
    freecb(uridata, (uint32_t)instCnt);
  }
}

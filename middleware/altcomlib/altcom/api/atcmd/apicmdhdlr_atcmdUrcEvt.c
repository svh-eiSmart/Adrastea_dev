...
void atcmdCheckInitCbCtrl(void) {
  ...
  if (!isMsgCbTblInit) {
    ...
    ret = alt_osal_create_mutex(&gAtcmdUrcCbCtrl.tblMtx, NULL);
...
static void atcmdUrcEvt_job(void *arg) {
  ...
  evt = (char *)arg;
  alt_osal_lock_mutex(&gAtcmdUrcCbCtrl.tblMtx, ALT_OSAL_TIMEO_FEVR);
  ...
  alt_osal_unlock_mutex(&gAtcmdUrcCbCtrl.tblMtx);
  if (callback) {

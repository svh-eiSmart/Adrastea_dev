...
#define APICALLBACK_THRD_PRIO (ALT_OSAL_TASK_PRIO_NORMAL)
...
static blockset_t g_blk_settings[] = {{16, 16}, {32, 12},  {128, 5}, {256, 4},
                                      {512, 2}, {2064, 1}, {5120, 2}};
...
static evthdl_if_t g_apicmdhdlrs[] = {
    ...
    apicmdhdlr_repcellinfo_2g,
    apicmdhdlr_repquality_2g,
#endif /* __ENABLE_LTE_API__ */
#ifdef __ENABLE_COAP_API__
    apicmdhdlr_cmdevt,
    apirsthdlr_cmdevt,
...
#endif
#ifdef __ENABLE_HTTP_API__
extern void http_callback_init(void);
#endif /* __ENABLE_HTTP_API__ */
struct apicmd_moduleinfo_s g_moduleInfo[] = {
...
#endif /* __ENABLE_COAP_API__ */
#ifdef __ENABLE_HTTP_API__
    {APIMODULEID_HTTP, HTTPVER_MAJOR, HTTPVER_MINOR},
#endif /* __ENABLE_HTTP_API__ */
#ifdef __ENABLE_LTE_API__
    ...
    {APIMODULEID_IO, IOVER_MAJOR, IOVER_MINOR},
#endif /* __ENABLE_IO_API__ */
#ifdef __ENABLE_FS_API__
    {APIMODULEID_FS, FSVER_MAJOR, FSVER_MINOR},
#endif /* __ENABLE_FS_API__ */
#ifdef __ENABLE_SMS_API__
    {APIMODULEID_SMS, SMSVER_MAJOR, SMSVER_MINOR},
#endif /* __ENABLE_SMS_API__ */
};
...
static int32_t bufferpool_initialize(bufmgmtcfg_t *bufMgmtCfg) {
  int32_t ret = -1;
  blockset_t *pset;
  uint8_t psetNum;
  mem_if_t g_memif = {buffpool_create, buffpool_delete, buffpool_alloc, buffpool_free,
                      buffpool_showstatistics};

  DBGIF_LOG1_NORMAL("Use %s buffpool interface.\n", bufMgmtCfg->mem_if ? "application" : "default");
  ret = buffpoolwrapper_config_memif(bufMgmtCfg->mem_if ? bufMgmtCfg->mem_if : &g_memif);
  if (0 > ret) {
    DBGIF_LOG1_ERROR("buffpoolwrapper_config_memif() error :%ld.\n", ret);
    return ret;
  }

  pset = NULL != bufMgmtCfg->blkCfg.blksetCfg ? (blockset_t *)bufMgmtCfg->blkCfg.blksetCfg
                                              : &g_blk_settings[0];
  psetNum = NULL != bufMgmtCfg->blkCfg.blksetCfg ? bufMgmtCfg->blkCfg.blksetNum : BLOCKSETLIST_NUM;
  DBGIF_LOG1_NORMAL("Use %s block configuration.\n",
                    NULL != bufMgmtCfg->blkCfg.blksetCfg ? "application" : "default");
  if (pset->blkSize) {
...
static int32_t apicmdgw_initialize(bool is_postpone_evt, bool is_bypass_echo) {
  int32_t ret;
...
static CODE int32_t lte_buildmain(FAR void *arg) {
  ...
  altcom_SetCallbackReg(initCfg->cbreg_only);
  ...
  ret = DbgIf_Init(initCfg->loglock_if);
  if (ret < 0) {
    goto errout;
  }
  ...
  ret = bufferpool_initialize(&initCfg->bufMgmtCfg);
  if (ret < 0) {
    goto errout_with_loginit;
  }
  ...
  ret = apicmdgw_initialize(initCfg->postpone_evt, initCfg->cbreg_only);
  if (ret < 0) {

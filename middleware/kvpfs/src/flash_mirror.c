...
static alt_osal_mutex_handle mirror_data_mtx = NULL;

#define LOCK() alt_osal_assert(alt_osal_lock_mutex(&mirror_data_mtx, ALT_OSAL_TIMEO_FEVR) == 0);

#define UNLOCK() alt_osal_assert(alt_osal_unlock_mutex(&mirror_data_mtx) == 0);
...
int32_t compact_flash(void) {
  int32_t dst_idx = 0;
  int8_t is_eof = 1;
  uint8_t *mirror = (uint8_t*)flm_info.mirror;
  uint32_t env_size = flm_info.env_size;
  delete_value_mirror(CRC_KEYNAME); // delete CRC before compact

  LOCK();
  for (size_t src_idx = 0; src_idx < env_size; src_idx++)
  {
      if(mirror[src_idx] == 0xff)
      {
          // reached 0xff means reached last byte
          break;
      }
      if(mirror[src_idx] != 0)
      {
          mirror[dst_idx++] = mirror[src_idx];
          is_eof = 1;
      }
      else
      {
  ...
  flm_info.num_of_zero = 0; // this function should eliminate every zero.
  flm_info.used_byte = dst_idx;
  memset((void *)(mirror+dst_idx), 0xff, env_size-dst_idx);
  flm_info.do_erase = 1;
  UNLOCK();
  return 0;
}
...
int32_t load_flash_content(void) {
  ...
  if (primary_ret != 0 && backup_ret == 0) {
    ...
    erase_flash(PRIMARY);
    flm_info.flash_write(flm_info.mirror, flm_info.env_primary, flm_info.env_size - 1, 1);
    ...
    erase_flash(BACKUP);
    flm_info.flash_write(flm_info.mirror, flm_info.env_backup, flm_info.env_size - 1, 1);
  } else if (primary_ret != 0 && backup_ret != 0) {
...
int32_t write_to_flash(void) {
  ...
  if (flm_info.do_erase) erase_flash(PRIMARY);
  flm_info.flash_write(flm_info.mirror, flm_info.env_primary, flm_info.env_size - 1, 1);

  if (flm_info.do_erase) erase_flash(BACKUP);
  flm_info.flash_write(flm_info.mirror, flm_info.env_backup, flm_info.env_size - 1, 1);
...
static void determin_kvpfs_secter(struct flash_mirror_info *mirro_info) {
  int8_t *backup_addr;
  mirro_info->env_primary =
      (int8_t *)(KVPFS_BASE_ADDR & SFLASH_4K_MASK);  // align primary bank to start of 4k
  ...
  if (backup_addr < mirro_info->env_primary + ENV_SIZE)
    backup_addr += SFLASH_4K_ERASE_SECTOR_SIZE;  // if backup is within the range of primary, round
                                                 // it to next 4K
...
void init_flash_mirror(struct flash_mirror_info **mirror_info) {
  ...
  if (alt_osal_create_mutex(&mirror_data_mtx, NULL)) {
    alt_osal_assert(0);
  }

  DRV_FLASH_Initialize();
  ...
  flm_info.flash_write = DRV_FLASH_Write;
  flm_info.flash_erase = DRV_FLASH_Erase_Sector;
...
void init_flash_mirror_from_stateful(void) {
  ...
  if (mirror_data_mtx == NULL) alt_osal_assert(0);
  DRV_FLASH_Initialize();
}

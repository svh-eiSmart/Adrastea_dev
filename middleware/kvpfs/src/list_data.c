...
#define LOCK() alt_osal_assert(alt_osal_lock_mutex(&list_data_mtx, ALT_OSAL_TIMEO_FEVR) == 0);

#define UNLOCK() alt_osal_assert(alt_osal_unlock_mutex(&list_data_mtx) == 0);

static struct list_data_info ldt_info = {0};
static alt_osal_mutex_handle list_data_mtx = NULL;
env_list *head = NULL;
env_list *cur = NULL;
...
void init_list_data(struct list_data_info **ldt) {
  ...
  if (alt_osal_create_mutex(&list_data_mtx, NULL)) {
    alt_osal_assert(0);
  }

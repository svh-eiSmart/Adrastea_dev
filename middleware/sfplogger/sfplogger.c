...
int sfp_log(int param1, int param2, int param3, unsigned int severitymoduleid,
  ...
  *(sfplogproto) &= ~SFP_MSB_SFP_TYPE_MASK;
  ...
  // TODO
  data = alt_osal_get_tick_count();  // time in msec - will wrap around every 49.7 days
...
int sfp_log_str(SFP_log_module_id_t moduleid, char severity, char *logstring) {
  ...
  *sfplogpt =
  ...
  timedata = htonl(alt_osal_get_tick_count());  // time in msec - will wrap around every 49.7 days
...
void sfplogger_clear_buffers(char *buffer_to_clear, char *pcWriteBuffer, size_t xWriteBufferLen,
  ...
  for (i = first; i < end; i++) {
    ...
    while (sfpdbasearr[i].lock != 0) alt_osal_sleep_task(1);
    portENTER_CRITICAL();
...
int sfplogger_create_db(int size) {
  ...
  if ((ret >= 0) && (sfpDatabaseEnabled == 1)) {
    sfpdbasearr[ret].sfpdbp = ALT_OSAL_MALLOC(size);
...
int sfplogger_resize_db(int moduleid, unsigned int size, int write_conf_to_file) {
  ...
  if (size <
    ...
    ALT_OSAL_FREE(placeholder);
    return 0;
  }

  char *new_db_buffer = ALT_OSAL_MALLOC(size);
  ...
  if (new_db_buffer == 0) {
    ...
    while (sfpdbasearr[dbid].lock != 0) alt_osal_sleep_task(1);
    portENTER_CRITICAL();
    ...
    ALT_OSAL_FREE(sfpdbasearr[dbid].sfpdbp);
